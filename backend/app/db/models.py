...
from sqlalchemy import BigInteger, Boolean, Column, DateTime, Float, ForeignKey, Integer, String, Table, Text, UniqueConstraint, func
...
class Asset(Base, TimestampMixin):
    ...
    processing_status: Mapped[str] = mapped_column(String(32), default="queued", index=True, nullable=False)
    visibility: Mapped[str] = mapped_column(String(32), default="private", nullable=False)

    folders: Mapped[list[Folder]] = relationship("Folder", secondary=asset_folders, back_populates="assets")
    tags: Mapped[list[Tag]] = relationship("Tag", secondary=asset_tags, back_populates="assets")
    extractions: Mapped[list[AssetExtraction]] = relationship("AssetExtraction", back_populates="asset", cascade="all,delete-orphan")
    chunks: Mapped[list[AssetChunk]] = relationship("AssetChunk", back_populates="asset", cascade="all,delete-orphan")
    shares: Mapped[list[Share]] = relationship("Share", back_populates="asset", cascade="all,delete-orphan")
    jobs: Mapped[list[IngestionJob]] = relationship("IngestionJob", back_populates="asset", cascade="all,delete-orphan")
...
class Folder(Base, TimestampMixin):
    ...
    assets: Mapped[list[Asset]] = relationship("Asset", secondary=asset_folders, back_populates="folders")
...
class AssetExtraction(Base):
    ...
    chunks: Mapped[list[AssetChunk]] = relationship("AssetChunk", back_populates="extraction", cascade="all,delete-orphan")
...
class AssetChunk(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    asset_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("assets.id", ondelete="CASCADE"), index=True, nullable=False)
    extraction_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("asset_extractions.id", ondelete="CASCADE"), index=True)
    qdrant_point_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), index=True, nullable=False)
    ...
    extraction: Mapped[AssetExtraction | None] = relationship("AssetExtraction", back_populates="chunks")
